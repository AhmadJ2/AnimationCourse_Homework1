...
IGL_INLINE void Renderer::init(igl::opengl::glfw::Viewer* viewer,int coresNum, igl::opengl::glfw::imgui::ImGuiMenu* _menu)
{
	...
	auto& key_down = [&](GLFWwindow* window, unsigned int key, int mod)->bool
	{
		switch (key)
		{
		case ' ':
	...
	callback_key_down = 
		(std::function<bool(GLFWwindow* window, unsigned int key, int modifiers)>)key_down;
}
...
void Renderer::MouseProcessing(int button)
{
	...
	if (scn->isPicked )
	{
		...
		{
			scn->data().MyRotate_in_system(Eigen::Vector3d(1,0,0),yrel/100.0);
			scn->data().MyRotate_in_system(Eigen::Vector3d(0, 1, 0), xrel / 100.0);
	...
	else
	{
		...
		{
			scn->MyRotate_in_system(Eigen::Vector3d(1, 0, 0), yrel / 100.0);
			scn->MyRotate_in_system(Eigen::Vector3d(0, 1, 0), xrel / 100.0);
			/*
